...
if (sessionStorage.getItem('x-rucio-auth-token') !== null) {
    token = sessionStorage.getItem('x-rucio-auth-token');
}
if (sessionStorage.getItem('rucio-selected-account') !== null) {
    account = sessionStorage.getItem('rucio-selected-account');
} else {
    // handle path
    sessionStorage.setItem('rucio-selected-account', account);
}
if (sessionStorage.getItem('rucio-selected-vo') !== null) {
    vo = sessionStorage.getItem('rucio-selected-vo');
} else {
    sessionStorage.setItem('rucio-selected-vo', vo);
}

var available_accounts = sessionStorage.getItem('rucio-available-accounts').split(' ');
...
function save_account_attr(acc) {
    r.list_account_attributes({
        ...
        success: function(attributes) {
            sessionStorage.setItem('rucio-account-attr', JSON.stringify(attributes));
...
function set_account(acc) {
    if (sessionStorage.getItem('x-rucio-auth-type') != 'x509'){
        sessionStorage.setItem('x-rucio-auth-token', "");
        sessionStorage.setItem('rucio-requested-path', window.location.href);
        ...
        r.get_auth_token_x509({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                sessionStorage.setItem('rucio-selected-account', acc);
                sessionStorage.setItem('x-rucio-auth-token', new_token);
                current_time = parseInt((new Date).getTime()/1000);
                sessionStorage.setItem('rucio-auth-token-created-at', current_time);
...
function check_attributes() {
    if (sessionStorage.getItem('rucio-account-attr') == null) {
        return;
    }
    attrs = JSON.parse(sessionStorage.getItem('rucio-account-attr'));
