...
function link_to_fts(e) {
    ...
    r.get_request_by_did({'scope': scope,
                          ...
                          success: function(data){
                              ...
                              if (['DONE', 'FAILED', 'SUBMITTED'].includes(data['state'])) {
...
function link_to_new_atlas_dashboard(file, scope, rse) {
    ...
    var link = "https://monit-grafana.cern.ch/d/FtSFfwdmk/ddm-transfers?panelId=118&fullscreen&orgId=17&var-binning=$__auto_interval_binning&var-groupby=dst_cloud&var-activity=All&var-src_cloud=All&var-src_site=All&var-src_country=All&var-src_endpoint=All&var-columns=src_cloud&var-dst_cloud=All&var-dst_endpoint=";
...
function load_examine() {
    ...
    r.examine_rule({
        'rule_id': url_param('rule_id'),
        success: function(data) {
            $("#examine_details").append("<table id=\"dt_examine\" class=\"compact stripe order-column cell-border\" style=\"word-wrap: break-word;\"><thead><th>Filename</th><th>RSEs</th><th>Attempts</th><th>Last Retry</th><th>Last Error</th><th>Last Source</th><th>Available Source</th><th>Blocklisted Sources</th></thead><tfoot><th>Filename</th><th>RSEs</th><th>Attempts</th><th>Last Retry</th><th>Last Error</th><th>Last Source</th><th>Available Source</th><th>Blocklisted Sources</th></tfoot></table>");
            ...
            $.each(data['transfers'], function(index, lock) {
                ...
                $.each(lock['sources'], function(index, src) {
                    if (src[1] == true) {
                        ...
                        blocklisted_srcs += src[0] + ', ';
                    }
                });
                lock['available_srcs'] = available_srcs.slice(0, -2);
                lock['blocklisted_srcs'] = blocklisted_srcs.slice(0, -2);
                dt.row.add(lock);
            });
            dt.order([0, 'asc']).draw();
            $("#show_examine").html("");
